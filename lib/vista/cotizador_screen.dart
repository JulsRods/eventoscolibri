...
import 'package:clase5/vista/UpdateCotizar_Screen.dart';
import 'package:clase5/vista/AddNewCotizar_Screen.dart';
import 'package:clase5/vista/registro_cotizaciones_screen.dart';
import 'package:clase5/vista/registro_cotizacionesU_screen.dart';
...
class _CotizarScreenState extends State<CotizarScreen> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      CotizacionService service = CotizacionService();
  ...
  void _calcularCotizacion() {
    if (_formKey.currentState!.validate()) {
      ...
      _preguntarEnviarWhatsApp(_cotizacion.toStringAsFixed(2));
    }
  }
  ...
  void _guardarCotizacion() {
    ...
    CotizacionService().addCotizacion(nuevaCotizacion).then((_) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _preguntarEnviarWhatsApp(String cotizacion) async {
    bool? enviar = await showDialog<bool>(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: Text('Enviar por WhatsApp'),
          content: Text('¿Deseas enviar la cotización por WhatsApp?'),
          actions: [
            TextButton(
              onPressed: () {
    ...
    if (enviar == true) {
      final Uri whatsappUri = Uri(
        ...
        host: 'wa.me',
        queryParameters: {
          'text': 'El total de su cotización es: $cotizacion Lps',
        },
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Cotización de Eventos'),
        leading: IconButton(
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => RegistroCotizacionesScreen()),
              );
            },
          ),
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => RegistroCotizacionesUScreen()),
              );
            },
          ),
        ],
